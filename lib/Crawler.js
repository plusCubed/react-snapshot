...
var Crawler = function () {
  function Crawler(baseUrl, snapshotDelay, options) {
    ...
    this.stripStyle = options.stripStyle;
    this.processed = {};
  ...
  _createClass(Crawler, [{
    ...
    value: function snap() {
      ...
      return (0, _snapshot2.default)(this.protocol, this.host, urlPath, this.snapshotDelay).then(function (window) {
        ...
        if (_this.stripStyle) {
          Array.from(window.document.querySelectorAll('style')).forEach(function (link) {
            if (link.hasAttribute(_this.stripStyle)) link.remove();
          });
        }
